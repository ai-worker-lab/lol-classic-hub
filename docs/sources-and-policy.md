...
5. 공개 제품명·도메인·소셜 계정·검색 태그에는 `Riot`, `League of Legends`, `LoL`, `롤`, 챔피언명 등 Riot 표장을 넣지 않는다. 제품명과 메타 제목은 일반 명칭 `클래식 노트`로 유지한다.
6. 광고·결제·후원·구독·유료 기능은 넣지 않는다. 수익화는 Developer Portal 등록과 `Approved` 또는 `Acknowledged` 상태를 확인한 뒤 별도 정책 검토를 거친다.
7. Developer Portal 제품 등록·감사는 공개와 병행하고, 아래 두 고지를 사용자가 쉽게 볼 수 있는 위치에 표시한다. 등록 완료 전에는 완료된 것으로 표현하지 않는다.
...
- 사용자 노출 제품명과 메타 제목은 일반 명칭 `클래식 노트`로 바꿨다. GitHub Pages의 등록 도메인은 `ai-worker-lab.github.io`이고 `lol-classic-hub`는 저장소 경로이므로, 도메인·소셜 계정 등록 금지 조항이 이 경로를 직접 금지한다고 단정할 근거는 확인되지 않았다. 다만 Riot 감사 피드백이 있으면 일반 slug로 이전한다.
...
| 공개 제품명·메타 제목 | 조건부 진행 가능 | `클래식 노트` 일반 명칭을 사용하고 Riot 표장을 SEO 키워드로 등록하지 않는다. 기존 저장소 경로는 Portal 감사 피드백 시 일반 slug로 이전한다. |
...
- 사용자 노출 제품명·메타 제목의 일반 명칭 전환: 완료 (`클래식 노트`)
- `3.13.24`와 현행 Classic 각 항목의 일치 여부: 확인 중
- 공개 직전 Riot 정책 변경 여부: 확인 중

Developer Portal 등록·감사는 AIW-184에서 공개와 병행한다. 수익화는 등록·감사와 별도 정책 검토가 끝나기 전까지 금지한다.