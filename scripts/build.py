...
def image_src(kind: str, entry: dict[str, object]) -> str:
    ...
    return href(f"/assets/riot-data-dragon/{VERSION}/img/{kind}/{filename}")
...
def data_cards(key: str, values: list[dict[str, object]]) -> str:
    ...
    for entry in sorted_values:
        ...
        cards.append(f'<article class="data-card"><img src="{image_src(kind, entry)}" alt="{name} 아이콘" width="56" height="56" loading="lazy"><div><h3>{name}</h3><div class="meta">{html.escape(meta)}</div></div><p>{html.escape(detail) or "설명 없음"}</p></article>')
...
<a class="card" href="{href('/builder/')}"><span class="tag">브라우저 전용</span><h3>빌드 메모 저장</h3><p>계정 없이 이 브라우저에 빌드를 저장</p></a></div></section>"""
    ...
    for path, title, key, noun in refs:
        values = datasets[key]
        body = f'<section class="hero"><p class="eyebrow">Data Dragon {VERSION}</p><h1>{title}</h1><p class="lede">공식 {LOCALE} 역사 데이터와 아이콘을 그대로 확인합니다. 현행 League Classic 동일성은 보증하지 않습니다.</p></section>{source_box(title, key, len(values))}<section><h2>{noun} 데이터</h2>{data_cards(key, values)}</section>'
